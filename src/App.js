...
import ThumbnailItem from './components/ThummbnailItem'
...
class App extends Component {
  ...
  checkingCorrectImage = id => {
    ...
    const findingIdOfObject = imagesList.filter(
      each => each.imageUrl === matchImage,
    )

    if (findingIdOfObject[0].id === id) {
      this.setState(prevState => ({score: prevState.score + 1}))

      const randomObject =
        imagesList[Math.floor(Math.random() * imagesList.length)]

      this.setState({matchImage: randomObject.imageUrl})
    } else {
      this.setState(prevState => ({isGameEnded: !prevState.isGameEnded}))
      clearInterval(this.timeInterval)
    }
  }
  ...
  render() {
    const {matchImage, activeTab, score, isGameEnded, time} = this.state
        ...
        {isGameEnded ? (
          <div className="game-result-container">
            <img
              ...
              alt="trophy"
            />
            <p className="match-score">YOUR SCORE</p>
          ...
          <div className="main-section">
            <img src={matchImage} className="match-image" alt="match" />
            <ul className="tabs-container">
              {tabsList.map(each => (
                <TabItem
                  tabDetails={each}
                  key={each.tabId}
                  selectingTab={this.selectingTab}
            ...
            <ul className="thumbnails-container">
              {filteredTabItems.map(each => (
                <ThumbnailItem
                  imageObject={each}
                  key={each.id}
                  checkingCorrectImage={this.checkingCorrectImage}
                />
              ))}
            </ul>
          </div>
